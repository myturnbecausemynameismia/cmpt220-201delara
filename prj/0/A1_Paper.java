...
public class A1_Paper {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < (numOfEach.length - 1); i++) {
	...
	public static float tapeLength(int[] numOfEach) {
		...
		float[][] dimensions;
		dimensions = new float[numOfEach.length - 1][1];
		// ^^ dimensions[i][0] = x
		// ^^ dimensions[i][1] = y
		...
		float x = (float) Math.pow(2, -(5 / 4));
		float y = (float) Math.pow(2, -(3 / 4));
		
		//init total x & y to compare later
		float totalX = 0;
		float totalY = 0;
		...
		for(int i = 2; i < (numOfEach.length - 1); i++) {
			//checks if dimensions are good
			if((totalY >= a1Y) && (totalX >= a1X))
				return tapeLength;
			//if they have an A2, add tapelength
			else if(numOfEach[0] > 0) {
				tapeLength += y;
				totalY += y * numOfEach[0];
				totalX += x * numOfEach[0];
				if(numOfEach[0] >= 2) {
					tapeLength += y;
					return tapeLength;
				}
			...
			//check for A3
			else if(numOfEach[1] > 0) {
				tapeLength += x;
				totalX += x * numOfEach[1];
				totalY += (y / 2) * numOfEach[1];
				if(numOfEach[1] >= 4) {
					tapeLength += x + y;
					return tapeLength;
				}
