...
//	}
	...
	public void render(int xPos, int yPos, int tile, int color, int mirrorDir, int scale){
		...
		int xTile = tile % 32;
		int yTile = tile / 32; //<< so we know how high it is
		int tileOffset = (xTile << 3) + (yTile <<3) * sheet.width; //2^ 3 = 8
		for(int y = 0; y < 8; y++) {
			int ySheet = y;
			if (mirrorY)
				ySheet = 7 - y;
			...
			for(int x = 0; x < 8; x++) {
				...
				int col = (color >> (sheet.pixels[xSheet + ySheet * sheet.width + tileOffset] * 8)) & 255; //verfigy if bet bounds of collors 0-255
				if (col < 255){
